...
import { ChevronRight, ChevronLeft, Bike, Shield, CheckCircle2, Loader2, MapPin } from 'lucide-react'
import type { RootState } from '../../store'
import {
  nextStep, prevStep, setVehicle, setConfig, setOwner,
  setPlans, setLoading, setStep,
} from '../../store/quoteSlice'
import { MOCK_PLANS, NCB_OPTIONS } from '../../data/mockData'
import type { CoverageType } from '../../types'

const BIKE_MAKES = ['Honda', 'Hero', 'TVS', 'Bajaj', 'Royal Enfield', 'Yamaha', 'Suzuki', 'KTM', 'Kawasaki']

const BIKE_MODELS: Record<string, string[]> = {
  Honda:           ['Activa 6G', 'Shine', 'Unicorn', 'SP 125', 'Hornet 2.0', 'CB300R'],
  Hero:            ['Splendor Plus', 'HF Deluxe', 'Glamour', 'Xpulse 200', 'Passion Pro'],
  TVS:             ['Jupiter', 'Apache RTR 160', 'Ntorq 125', 'Raider 125', 'iQube'],
  Bajaj:           ['Pulsar NS200', 'Pulsar 150', 'Avenger Cruise 220', 'Dominar 400', 'CT100'],
  ...
  Suzuki:          ['Access 125', 'Burgman Street', 'Gixxer SF 250', 'V-Strom SX'],
  KTM:             ['Duke 200', 'Duke 390', 'RC 390', 'Adventure 390'],
  Kawasaki:        ['Ninja 300', 'Ninja 650', 'Z900', 'Versys-X 300'],
}
...
function Step1({ onNext }: { onNext: () => void }) {
  ...
  function handleNext() {
    if (policyType === 'renewal' && regNo.length < 9) {
      setError('Please enter a valid registration number (e.g. MH04BX1234)')
      return
    }
    setError('')
    ...
    <div className="flex flex-col items-center">
      ...
      {policyType === 'renewal' ? (
        <div className="w-full max-w-sm">
          <label className="block text-sm font-semibold text-navy-900 mb-2">Bike Registration Number</label>
          ...
          <p className="text-slate-400 text-xs mt-2">We'll fetch your bike details automatically</p>
        </div>
      ) : (
        <div className="w-full max-w-sm text-center">
          <div className="w-16 h-16 bg-teal-50 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <Bike size={28} className="text-teal-500" />
          </div>
          <p className="text-slate-600 text-sm leading-relaxed mb-4">
            We'll collect your bike details in the next step. Click Continue to proceed.
          </p>
        </div>
      )}
...
function Step2({ onNext, onBack }: { onNext: () => void; onBack: () => void }) {
  ...
  const models = make ? (BIKE_MODELS[make] ?? []) : []
  ...
  function handleNext() {
    if (!validate()) return
    dispatch(setVehicle({ make, model, year: Number(year), fuelType: 'Petrol', city }))
    onNext()
  }
    ...
    <div className="w-full max-w-lg mx-auto">
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2 sm:col-span-1">
          <label className="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wide">Bike Make *</label>
          <select className="cf-select" value={make} onChange={e => { setMake(e.target.value); setModel('') }}>
            <option value="">Select Make</option>
            {BIKE_MAKES.map(m => <option key={m} value={m}>{m}</option>)}
          </select>
      ...
      <div className="flex gap-3 mt-8">
        ...
        <button className="btn-primary flex-1" onClick={handleNext}>View Quotes <ChevronRight size={16} /></button>
      </div>
    </div>
  )
}
...
function Step3({ onNext, onBack }: { onNext: () => void; onBack: () => void }) {
  ...
  const coverageOptions: { type: CoverageType; title: string; desc: string; from: string; rec?: boolean }[] = [
    { type: 'comprehensive', title: 'Comprehensive', desc: 'Own damage + third party + personal accident cover', from: '₹1,800/yr', rec: true },
    { type: 'third_party',   title: 'Third Party',   desc: 'Mandatory cover for damage to others. Basic protection.', from: '₹714/yr' },
    { type: 'own_damage',    title: 'Own Damage Only', desc: 'Covers your bike damage only (requires valid TP policy)', from: '₹950/yr' },
  ]
    ...
    <div className="w-full max-w-lg mx-auto space-y-6">
      <div>
        ...
        <div className="space-y-2.5">
          {coverageOptions.map(opt => (
            <label key={opt.type} className={`flex items-start gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all ${coverage === opt.type ? 'border-navy-900 bg-navy-50' : 'border-slate-200 bg-white hover:border-slate-300'}`}>
              <input type="radio" className="mt-1 accent-navy-900" checked={coverage === opt.type} onChange={() => setCoverage(opt.type)} />
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className="font-semibold text-navy-900 text-sm">{opt.title}</span>
                  {opt.rec && <span className="badge bg-teal-100 text-teal-700 text-[10px]">Recommended</span>}
                </div>
      ...
      <div className="flex items-center justify-between p-4 rounded-xl bg-slate-50 border border-slate-200">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Affects your No Claim Bonus eligibility</p>
        </div>
      ...
      {!claimed && (
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {NCB_OPTIONS.map(opt => (
              <button key={opt.value} onClick={() => setNcb(opt.value)}
                className={`p-3 rounded-xl border-2 text-left transition-all ${ncb === opt.value ? 'border-teal-500 bg-teal-50' : 'border-slate-200 bg-white hover:border-teal-300'}`}>
                <div className="text-lg font-bold text-navy-900">{opt.value}%</div>
                <div className="text-[10px] text-slate-500 mt-0.5">{opt.value === 0 ? 'No NCB' : `${opt.value === 20 ? '1yr' : opt.value === 25 ? '2yr' : opt.value === 35 ? '3yr' : opt.value === 45 ? '4yr' : '5yr+'} claim-free`}</div>
              </button>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="w-full max-w-sm mx-auto">
      <div className="text-center mb-6">
        ...
        <p className="text-sm text-slate-500">Almost there! Enter your details to see personalised bike insurance quotes.</p>
      </div>
      ...
      <div className="flex gap-3 mt-6">
        ...
        <button className="btn-primary flex-1" onClick={handleSubmit} disabled={loading}>
          {loading ? <><Loader2 size={16} className="animate-spin" /> Fetching Quotes...</> : <>View Plans <ChevronRight size={16} /></>}
        </button>
      </div>
    </div>
  )
...
// ── Main ───────────────────────────────────────────────────────────────────
const STEPS = ['Registration', 'Bike Details', 'Coverage', 'Your Info']

export default function BikeQuoteFlow() {
  const dispatch = useDispatch()
  ...
  useEffect(() => {
    dispatch(setStep(1))
    dispatch(setConfig({ vehicleType: 'bike' }))
  }, [dispatch])
            ...
            {STEPS.map((label, i) => {
              ...
              return (
                <div key={label} className="flex items-center">
                  ...
                  {i < STEPS.length - 1 && (
                    <div className={`h-0.5 w-14 sm:w-20 mx-2 rounded-full transition-all duration-500 ${num < step ? 'bg-teal-500' : 'bg-slate-200'}`} />
                  )}
                </div>
              )
            })}
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-display font-bold text-navy-900">
            {step === 1 && "Let's start with your bike"}
            {step === 2 && 'Tell us about your bike'}
            {step === 3 && 'Choose your coverage'}
            {step === 4 && 'Almost done!'}
          </h1>
          <p className="text-slate-500 text-sm mt-1.5">
            {step === 1 && 'Renewing or buying a new bike policy?'}
        ...
        <div className="mt-6 flex items-center justify-center gap-6 text-xs text-slate-400">
          ...
          <span className="flex items-center gap-1.5"><Bike size={13} className="text-teal-500" /> Bike Specialist</span>
        </div>
      </div>
    </div>
  )
